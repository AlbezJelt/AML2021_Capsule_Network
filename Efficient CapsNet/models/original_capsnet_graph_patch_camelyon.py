...
def capsnet_graph(input_shape, num_classes, routing):
    """
...
def generator_graph(input_shape, num_classes):
    """
    ...
    input_shape: list
        network input shape
    """
    inputs = tf.keras.Input(16*num_classes)
...
def build_graph(input_shape, num_classes, mode, n_routing, verbose):
    """
    ...
    """
    inputs = tf.keras.Input(input_shape)
    y_true = tf.keras.Input(shape=(num_classes))
    noise = tf.keras.layers.Input(shape=(num_classes, 16))
    
    capsnet = capsnet_graph(input_shape, num_classes, routing=n_routing)
    ...
    generator = generator_graph(input_shape, num_classes)
