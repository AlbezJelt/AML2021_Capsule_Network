...
def kernel_tile(input, kernel, stride):
    ...
    tiled = tf.reshape(tiled, [batch_size, parent_spatial_size,
                       parent_spatial_size, kernel*kernel, n_capsules, -1])
...
def to_sparse(probs, spatial_routing_matrix, sparse_filler=tf.math.log(1e-20)):
    ...
    shape = [batch_size, parent_space_2,
    ...
    sparse = tf.reshape(sparse, [batch_size, parent_space,
                        parent_space, child_space_2, child_caps, parent_caps])

    # Checks
    # 1. Shape
    assert sparse.get_shape().as_list() == [
        batch_size, parent_space, parent_space, child_space_2, child_caps, parent_caps]
