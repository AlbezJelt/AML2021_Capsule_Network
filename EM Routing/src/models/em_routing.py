...
def m_step(rr, votes, activations_i, beta_v, beta_a, inverse_temperature):
    ...
    with tf.compat.v1.variable_scope("m_step") as scope:
        ...
        # layer_norm_factor
        cost_j_h = (beta_v + 0.5*tf.math.log(var_j)) * \
            rr_prime_sum * layer_norm_factor
		...
		cost_j_h = (beta_v + tf.math.log(stdv_j + FLAGS.epsilon)) * rr_prime_sum
		"""
...
def e_step(votes_ij, activations_j, mean_j, stdv_j, var_j, spatial_routing_matrix):
    ...
    with tf.compat.v1.variable_scope("e_step") as scope:
        ...
        """
		# In un-log space
		with tf.compat.v1.variable_scope("to_sparse_unlog") as scope:
			zz_unlog = tf.exp(zz)
			...
			zz_sparse_log = tf.math.log(zz_sparse_unlog + 1e-15) 
...
#       sparse_filler = -100
        ...
        with tf.compat.v1.variable_scope("softmax_across_parents") as scope:
            rr_sparse = utl.softmax_across_parents(
                zz_sparse, spatial_routing_matrix)
