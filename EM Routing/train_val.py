...
import dataset_factories as dsFactories
...
def main(args):
    ...
    dataset_size_train = dsFactories.get_dataset_size_train(FLAGS.dataset)
    dataset_size_val = dsFactories.get_dataset_size_validate(FLAGS.dataset)
    build_arch = dsFactories.get_dataset_architecture(FLAGS.dataset)
    num_classes = dsFactories.get_num_classes(FLAGS.dataset)
    create_inputs_train = dsFactories.get_create_inputs(
        FLAGS.dataset, mode="train")
    create_inputs_val = dsFactories.get_create_inputs(
        FLAGS.dataset, mode="validate")
 ...
 # *****************************************************************************
    ...
    with g_val.as_default():
        ...
        num_batches_val = int(
            dataset_size_val / FLAGS.batch_size * FLAGS.val_prop)
    ...
    #----- SESSION TRAIN -----#
    # Session settings
    config_train = tf.ConfigProto(allow_soft_placement=True,
                                  log_device_placement=False)
    config_train.gpu_options.allow_growth = True
    config_train.gpu_options.per_process_gpu_memory_fraction = 0.8
    sess_train = tf.Session(config=config_train, graph=g_train)
    ...
    #----- SESSION VALIDATION -----#
    config_val = tf.ConfigProto(allow_soft_placement=True,
                                log_device_placement=False)
    config_val.gpu_options.allow_growth = True
    config_train.gpu_options.per_process_gpu_memory_fraction = 0.8
    sess_val = tf.Session(config=config_val, graph=g_val)
    with g_val.as_default():
